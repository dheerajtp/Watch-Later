...
function RecomendationSingle({ movie, key, isAuthenticated }) {
  const image = `https://image.tmdb.org/t/p/w500${movie.poster_path}`;
  return (
    <Link to={`/movie/${movie.id}`}>
      <div className="py-5 flex justify-center items-center" key={key}>
        <div className="relative pl-1 flex justify-center rounded-xl hover:scale-105 duration-500 transform transition cursor-pointer">
          ...
          <div className="w-96 pb-2 bg-white rounded-xl shadow-xl z-10">
            ...
            <div className="px-2 py-1">
              ...
              {isAuthenticated ? (
                <button className="flex items-center justify-center mx-auto p-2 pl-5 pr-5 bg-transparent border-2 border-zinc-800 text-zinc-800 text-sm rounded-lg focus:border-4 focus:border-green-300">
                  Add To Watch Later
                </button>
              ) : (
                ""
...
export default RecomendationSingle;
