...
const WatchLaterSingleMovie = ({ movie, id }) => {
  const navigate = useNavigate();
  const watchLaterCollectionReference = collection(db, "watchlater");
  const [loading, setLoading] = useState(false);
  const image = `https://image.tmdb.org/t/p/w500${movie.movie.image}`;
  const { addToast } = useToasts();
  const removeFromWatchList = async () => {
    setLoading(true);
    const itemDoc = doc(watchLaterCollectionReference, movie.id);
    await deleteDoc(itemDoc);
    setLoading(false);
    navigate("/");
    addToast("Removed From Watch Later List..!!", { appearance: "success" });
  };
        ...
        <div className="relative pl-1 flex justify-center rounded-xl ">
          ...
          <div className="w-96 pb-2 bg-white rounded-xl shadow-xl z-10">
            ...
            <div className="px-2 py-1">
              <Link to={`/movie/${movie.movie.movieid}`}>
                  ...
                  <div className="flex items-center">
                    ...
                    <p className="text-gray-600 font-bold text-xs md:text-sm ml-1">
                      ...
                      <span className="text-gray-500 font-normal">
                        {" "}
                        ({movie.movie.votecount} Vote Count)
                      </span>
                    </p>
                  </div>
                </div>
              ...
              <p className="pb-1 md:pb-2 text-xs md:text-sm text-gray-500">
                {movie.movie.overview.substring(0, 250)}..
              </p>
...
export default WatchLaterSingleMovie;
